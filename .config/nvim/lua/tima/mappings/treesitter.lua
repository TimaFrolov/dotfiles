local ts_repeat_move = require('nvim-treesitter.textobjects.repeatable_move')

vim.keymap.set({ "n", "x", "o" }, ";", ts_repeat_move.repeat_last_move)
vim.keymap.set({ "n", "x", "o" }, ",", ts_repeat_move.repeat_last_move_opposite)

vim.keymap.set({ "n", "x", "o" }, "f", ts_repeat_move.builtin_f)
vim.keymap.set({ "n", "x", "o" }, "F", ts_repeat_move.builtin_F)
vim.keymap.set({ "n", "x", "o" }, "t", ts_repeat_move.builtin_t)
vim.keymap.set({ "n", "x", "o" }, "T", ts_repeat_move.builtin_T)

return {
  incremental_selection = {
    init_selection = "<leader>vk",
    node_incremental = "<leader>vk",
    scope_incremental = false,
    node_decremental = "<leader>vj",
  },
  textobjects = {
    select = {
      ["i="] = { query = "@assignment.inner" },
      ["a="] = { query = "@assignment.outer" },
      ["=h"] = { query = "@assignment.lhs" },
      ["=l"] = { query = "@assignment.rhs" },
      ["ib"] = { query = "@block.inner" },
      ["ab"] = { query = "@block.outer" },
      ["i@"] = { query = "@call.inner" },
      ["a@"] = { query = "@call.outer" },
      ["ic"] = { query = "@class.inner" },
      ["ac"] = { query = "@class.outer" },
      ["i/"] = { query = "@comment.inner" },
      ["a/"] = { query = "@comment.outer" },
      ["ii"] = { query = "@conditional.inner" },
      ["ai"] = { query = "@conditional.outer" },
      ["if"] = { query = "@function.inner" },
      ["af"] = { query = "@function.outer" },
      ["il"] = { query = "@loop.inner" },
      ["al"] = { query = "@loop.outer" },
      ["in"] = { query = "@number.inner" },
      ["ia"] = { query = "@parameter.inner" },
      ["aa"] = { query = "@parameter.outer" },
      ["ie"] = { query = "@regex.inner" },
      ["ae"] = { query = "@regex.outer" },
      ["ir"] = { query = "@return.inner" },
      ["ar"] = { query = "@return.outer" },
      ["is"] = { query = "@scope.inner" },
      ["as"] = { query = "@statement.outer" },
    },
    swap_next = {
      ["<leader>wnb"] = "@block.inner",
      ["<leader>wnc"] = "@class.outer",
      ["<leader>wni"] = "@conditional.inner",
      ["<leader>wnf"] = "@function.outer",
      ["<leader>wnn"] = "@number.outer",
      ["<leader>wna"] = "@parameter.inner",
      ["<leader>wne"] = "@regex.inner",
      ["<leader>wnr"] = "@return.inner",
      ["<leader>wns"] = "@statement.outer",
    },
    swap_previous = {
      ["<leader>wpb"] = "@block.inner",
      ["<leader>wpc"] = "@class.outer",
      ["<leader>wpi"] = "@conditional.inner",
      ["<leader>wpf"] = "@function.outer",
      ["<leader>wpa"] = "@parameter.inner",
      ["<leader>wpe"] = "@regex.inner",
      ["<leader>wpr"] = "@return.inner",
      ["<leader>wps"] = "@statement.outer",
    },
    move = {
      next_start = {
        ["]b"] = { query = "block.outer" },
        ["]@"] = { query = "call.outer" },
        ["]c"] = { query = "class.outer" },
        ["]/"] = { query = "comment.outer" },
        ["]i"] = { query = "conditional.outer" },
        ["]f"] = { query = "function.outer" },
        ["]l"] = { query = "loop.outer" },
        ["]n"] = { query = "number.inner" },
        ["]a"] = { query = "parameter.inner" },
        ["]e"] = { query = "regex.inner" },
        ["]r"] = { query = "return.outer" },
        ["]s"] = { query = "statement.outer" },
      },
      next_end = {
        ["]B"] = { query = "block.outer" },
        ["]C"] = { query = "class.outer" },
        ["]I"] = { query = "conditional.outer" },
        ["]F"] = { query = "function.outer" },
        ["]L"] = { query = "loop.outer" },
        ["]N"] = { query = "number.inner" },
        ["]A"] = { query = "parameter.inner" },
        ["]E"] = { query = "regex.inner" },
        ["]R"] = { query = "return.outer" },
        ["]S"] = { query = "statement.outer" },
      },
      previous_start = {
        ["[b"] = { query = "block.outer" },
        ["[@"] = { query = "call.outer" },
        ["[c"] = { query = "class.outer" },
        ["[/"] = { query = "comment.outer" },
        ["[i"] = { query = "conditional.outer" },
        ["[f"] = { query = "function.outer" },
        ["[l"] = { query = "loop.outer" },
        ["[n"] = { query = "number.inner" },
        ["[a"] = { query = "parameter.inner" },
        ["[e"] = { query = "regex.inner" },
        ["[r"] = { query = "return.outer" },
        ["[s"] = { query = "statement.outer" },
      },
      previous_end = {
        ["[B"] = { query = "block.outer" },
        ["[C"] = { query = "class.outer" },
        ["[I"] = { query = "conditional.outer" },
        ["[F"] = { query = "function.outer" },
        ["[L"] = { query = "loop.outer" },
        ["[N"] = { query = "number.inner" },
        ["[A"] = { query = "parameter.inner" },
        ["[E"] = { query = "regex.inner" },
        ["[R"] = { query = "return.outer" },
        ["[S"] = { query = "statement.outer" },
      },
    },
  },
}
